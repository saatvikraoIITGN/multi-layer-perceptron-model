...
def run_test_harness():
    ...
    logs_mlp = "logs_mlp/fit/vgg1_" + datetime.datetime.now().strftime("%H%M%S")
    tensorboard_callback = keras.callbacks.TensorBoard(log_dir=logs_mlp, histogram_freq=1)
    # ----|
    ...
    history = model.fit(train_it, steps_per_epoch=len(train_it), validation_data=test_it, validation_steps=len(test_it), epochs=15, verbose=2, callbacks=[tensorboard_callback])
    end_time = time.time()
