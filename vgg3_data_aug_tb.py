...
def run_test_harness():
	...
	logs_vgg3_data_aug = "logs_vgg3_data_aug/fit/vgg1_" + datetime.datetime.now().strftime("%H%M%S")
	tensorboard_callback = keras.callbacks.TensorBoard(log_dir=logs_vgg3_data_aug, histogram_freq=1)
    # ----|
	...
	# prepare iterators
	train_it = train_datagen.flow_from_directory('dataset_rats_vs_squirrels/train/', class_mode='binary', batch_size=64, target_size=(200, 200))
	test_it = test_datagen.flow_from_directory('dataset_rats_vs_squirrels/test/', class_mode='binary', batch_size=64, target_size=(200, 200))
	
	# fit model
	# ----
	start_time = time.time()
	history = model.fit(train_it, steps_per_epoch=len(train_it), validation_data=test_it, validation_steps=len(test_it), epochs=25, verbose=2, callbacks=[tensorboard_callback])
	end_time = time.time()
	...
	logdir = "logs_vgg3_data_aug/images"
